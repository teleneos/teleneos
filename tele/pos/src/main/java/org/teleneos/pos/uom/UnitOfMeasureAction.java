...
public class UnitOfMeasureAction extends DefaultAction implements
		ModelDriven<UnitOfMeasureActionModel> {
	...
	public ActionResult uomList() {
		model.setUoms(uomService.findByKeyword(model.getQ(), null, "ASC",
				model.getMax(), model.getPage() - 1));

		return new ActionResult("freemarker", "/view/pos/uom/uom-list.ftl");
	}

	@Action(name = "/add", method = HttpMethod.GET)
	public ActionResult addForm() {
		return new ActionResult("freemarker", "/view/pos/uom/uom-form.ftl");
	}
	...
	public ActionResult addUom() {
		...
		try {
			...
			addFieldError("uom.name", "Code already in use");
		}
