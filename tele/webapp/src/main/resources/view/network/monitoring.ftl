<html>
	...
	<body>
		...
		function updateData() {
			$.getJSON('<@s.url />/traffic.json', function(data) {
				var inTraffic = data.histories[0];
				var outTraffic = data.histories[1];
				var i = inTraffic.result.length;
				...
				for (r in inTraffic.result) {
					r = inTraffic.result[r];
				...
				avg = avg / outTraffic.result.length;
