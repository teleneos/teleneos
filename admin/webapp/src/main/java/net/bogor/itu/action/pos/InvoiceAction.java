...
import net.bogor.itu.service.pos.InvoiceService;
...
public class InvoiceAction extends DefaultAction implements
	...
	public ActionResult InvoiceList() {
		model.setInvoices(invoiceService.findByKeyword(
				model.getQ(), null, "ASC", model.getMax(), model.getPage() - 1));

		return new ActionResult("freemarker", "/view/pos/invoice/invoice-list.ftl");
	}

	@Action(name = "/add", method = HttpMethod.GET)
	public ActionResult addForm() {
		return new ActionResult("freemarker", "/view/pos/invoice/invoice-form.ftl");
	}
	...
	public ActionResult formDetail() {
		...
		model.setInvoiceDetails(invoiceDetailService.findByKeyword(
				model.getInvoice().getId(), 0, 0));

		model.setInvoice(invoiceService.findById(model
				.getInvoice().getId()));

		model.setPurchaseOrderDetails(purchaseOrderDetailService.findByKeyword(
				model.getInvoice().getPurchaseOrder().getId(), 0, 0));

		return new ActionResult("freemarker", "/view/pos/invoice/invoice-detail-form.ftl");
	}
	...
	public ActionResult addInvoice() {
		...
		for (int j = 0; j < i; j++) {
			...
			in.setInvoice(invoiceService.findById(model
					.getInvoice().getId()));
